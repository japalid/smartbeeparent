...
class DetailTips extends React.Component {
  ...
  _getBlog() {
      ...
      AsyncStorage.getItem("auth-key")
        .then(async (res) => {
          if (res !== null) {
            let resp = await request.bulletinTipsId(res,this.props.navigation.getParam("id"));
            if(resp.status==200) {
            ...
            <View style={{flexDirection:'row',backgroundColor:'#AD90CA',height:70 }}>
                  <View style={{marginTop: (Platform.OS) == 'ios' ? 30 : 0,alignItems:'center',justifyContent:'space-between',flexDirection:'row',width:width}}>
                      ...
                      <View style={{margin:10}}>
                          <Text style={{color:'#fff',fontSize:16,fontWeight:'bold'}}>Tips</Text>
                      </View>
                      </View>
